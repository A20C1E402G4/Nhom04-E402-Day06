...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        if vehicle:
            ...
            st.markdown(
                f"<p style='font-size: 1.35rem; font-weight: 700; color: #0088cc;'>Giá: {fmt_vnd(vehicle.get('price_without_battery_vnd'))}</p>",
                unsafe_allow_html=True)
            st.divider()
        ...
        if st.button("🗑️ Làm mới hội thoại", use_container_width=True):
            ...
            for key in ("session_id", "user_context", "chat_history", "last_vehicle", "loan_term", "is_processing",
                        "pending_input"):
...
def render_loan_sidebar(vehicle: dict[str, Any]) -> None:
    ...
    if int(draft_pct) != committed_pct or int(draft_term) != committed_term:
        if st.button("✅ Lưu làm mặc định", use_container_width=True):
            telemetry_logger.log_correction({"field": "loan_percentage", "from": committed_pct, "to": int(draft_pct),
        ...
        with st.chat_message("assistant"):
            with st.status("VSSA đang kết nối...", expanded=True) as status:
                try:
                    ...
                    state_input = {"messages": [HumanMessage(content=user_msg)],
                                   "user_context": st.session_state.user_context, "tool_turns": 0}
                    ...
                    if payload:
                        ...
                        if "loan" in payload:
                            l = payload["loan"];
                            st.session_state.loan_term = int(l.get("duration_months", 96))
